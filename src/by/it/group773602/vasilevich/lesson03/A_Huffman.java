package by.it.group773602.vasilevich.lesson03;
...
public class A_Huffman {
    ...
    String encode(File file) throws FileNotFoundException {
        ...
        for (Character character: s.toCharArray()) {
            if (count.containsKey(character)) {
                Integer amount = count.get(character) + 1;
                count.put(character, amount);
            }
            else {count.put(character, 1);}
        }

        //2. перенесем все символы в приоритетную очередь в виде листьев
        PriorityQueue<Node> priorityQueue = new PriorityQueue<>();
        count.forEach(((character, amount) -> priorityQueue.add(new LeafNode(amount, character))));
        ...
        while (priorityQueue.size() != 1) {
            Node firstLeaf = priorityQueue.poll();
            Node secondLeaf = priorityQueue.poll();
            priorityQueue.add(new InternalNode(firstLeaf, secondLeaf));
        }
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        File f = new File(root + "by/it/group773602/vasilevich/lesson03/dataHuffman.txt");
