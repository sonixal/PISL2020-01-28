package by.it.group773601.provalionok.lesson3;
...
public class A_Huffman {
    ...
    String encode(File file) throws FileNotFoundException {
        ...
        for (int i = 0; i < s.length(); i++) {
            ...
            if (count.containsKey(symb))
                count.put(symb, count.get(symb) + 1);
            else count.put(symb, 1);
        }

        Queue<Node> priorityQueue = new PriorityQueue<>();
        // Создаем очередь из LeafNode
        for (Map.Entry temp : count.entrySet()) {
            priorityQueue.add(new A_Huffman.LeafNode((int) temp.getValue(), (char) temp.getKey()));
        }

        // Генерируем дерево Хаффмена
        while (priorityQueue.size() > 1) {
            A_Huffman.Node leftNode = priorityQueue.peek();
            priorityQueue.remove();
            A_Huffman.Node rightNode = priorityQueue.peek();
            priorityQueue.remove();
            priorityQueue.add(new A_Huffman.InternalNode(leftNode, rightNode));
        }
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        File f = new File(root + "by/it/group773601/provalionok/lesson3/dataHuffman.txt");
